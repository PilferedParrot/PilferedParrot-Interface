...
function renderHeader() {
  ...
  $("#harnessReturn").textContent = "↩ Back to session";
  $("#harnessReturn").setAttribute("aria-label", "Back to session");
...
function render() {
  ...
  $("#prompt").placeholder = harnessChild
    ? "This saved task is read-only. Use Back to session to continue"
...
let providerRemoveTarget = null;
function removeProvider(provider) {
  providerRemoveTarget = provider;
  $("#providerRemoveNote").textContent = `Remove ${providerLabel(provider)}? You can restore it from Add provider. Your saved conversations stay on this computer.`;
  $("#providerRemoveDialog").showModal();
}

$("#providerRemoveForm").addEventListener("submit", async (event) => {
  if (event.submitter?.value === "cancel") return;
  event.preventDefault();
  const provider = providerRemoveTarget;
  const button = $("#confirmProviderRemove");
  if (!provider || button.disabled) return;
  button.disabled = true;
  try {
    await api("/api/providers/remove", { method: "POST", body: JSON.stringify({ provider }) });
    $("#providerRemoveDialog").close();
    await refreshState();
...
$("#providerRemoveDialog").addEventListener("close", () => {
  // A queued close event may arrive after the user has opened another confirmation.
  if (!$("#providerRemoveDialog").open) providerRemoveTarget = null;
});

// Closing a dialog must never submit its form or run the primary action.
...
$("#providerConnectionList").addEventListener("keydown", (event) => {
  ...
  event.preventDefault();
  const provider = input.dataset.providerAuthCode;
  $("#providerConnectionList").querySelector(`[data-provider-code="${provider}"]`)?.click();
});
