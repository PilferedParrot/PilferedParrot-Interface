...
10/11 x64 preview. Assets, checksums, and verification are linked from the
[0.8.0 release](https://github.com/PilferedParrot/PilferedParrot-Interface/releases/tag/v0.8.0).
The 0.7.1 release and its archives remain immutable.
...
- An opt-in ACP Work preview for Codex and Claude streams text, tool details and diffs, and
  ...
  model, effort, and mode choices. The legacy engine remains the default. Windows provider accounts
  and real provider-CLI sign-in are not live-certified by the package gate.
...
Download the source archive or Windows x64 ZIP and verify it
